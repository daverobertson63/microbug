...
def main_single_linux(source_file, dest_file, tmp_directory, cleanup=False):
    """
...
#
#  DHR - Added a windows version here to make it work on Windows with Arduino installed. 
#
def main_single_windows(source_file):
    """
    Compiles a single source file.
    Creates a dest_file - which is a .hex file.
    May work inside tmp_directoy
    ...
    print "Converting Python file to C++ Arduino file"
    ...
    print "Creating python file in Arduino dir"
    ...
    print "Creating Arduino compilation C++ File as microbug.ino"

    f = open(basedir+"/microbug.ino", "w")
    f.write(y)
    f.close()
    ...
    # Now attempt to run avrdude and all that good stuff - or get the lad to it with the UI
    here = os.getcwd()
    os.chdir(basedir)
    os.system("echo Running AVRDUDE")
